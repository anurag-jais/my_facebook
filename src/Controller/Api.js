...
//const SENDGRID_API_KEY ="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";
//sgMail.setApiKey(process.env.SENDGRID_API_KEY);
sgMail.setApiKey(
  "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"
);
        ...
        User.findOne({
          ...
          .then(identifyButNotRegister => {
            ...
            if (identifyButNotRegister) {
              const otpForCompletingRegistration = Math.floor(
              ...
              User.findOne({
                ...
                .then(identified => {
                  const otpForRecoverAccount = Math.floor(
                    100000 + Math.random() * 900000
                  );
                  if (identified) {
                    console.log("updation start");
                    User.update(
                      { userotp: otpForRecoverAccount },
  ...
  User.findOne({
    ...
    .then(alreadyuser => {
      if (alreadyuser) {
        ...
        bcrypt
          .hash(req.body.userpassword, 12)
          .then(hashedPassword => {
            services
              .getSignInFunc(req.body, otp, hashedPassword)
              .then(result => {
                if (result) {
                  const msg = {
                    to: req.body.useremail,
                    from: "anurag@cronj.com",
                    subject: "OTP for verify registration through sendgrid",
                    html: "<h1>" + otp + "</h1>",
                    templateId: "d-2ef57777395a4d039ba4953d71bd8809",
                  ...
                  //     from: "anurag@cronj.com",
                  //     to: req.body.useremail,
                  //     subject: "OTP for registration through Sendgrid",
                  //     html: "<h1>" + otp + "</h1>"
                  //   },
                  //   function(error, info) {
                  //     if (error) {
  ...
  services
    ...
    .then(result => {
      ...
      if (result && result.useremailverifiedstatus === false) {
        ...
        //loadeduser = result
        bcrypt
          .compare(req.body.password, result.userpassword)
          .then(isMatch => {
            if (isMatch) {
...
exports.updatePassword = (req, res, next) => {
  bcrypt
    .hash(req.body.userpassword, 12)
    .then(updatedHashPassword => {
      services
        .updatePasswordFunc(req.body, updatedHashPassword)
        .then(updated => {
          if (updated) {
...
// const upload = multer({ dest: "hojaaupload" }).single("name");
exports.createPost = (req, res, next) => {
  // upload(req, res, function(err) {
  //   if (err instanceof multer.MulterError) {
  //     console.log("A multer error occured while uploading");
  //   } else if (err) {
  ...
  console.log("image", req.body.file.file);
  services
    .findUserFunc(req.body.userId)
    .then(user => {
      if (user) {
        console.log("userfound");
        console.log(user);
        services
          .createPostFunc(req, user)
          .then(postcreated => {
            if (postcreated) {
