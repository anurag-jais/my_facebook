    ...
    handleSubmit = e => {
        ...
        this.props.form.validateFields((err, values) => {
            if (!err) {
                ...
                axios.post('http://localhost:3007/api/login', values)
                    .then(response => {
    ...
    render() {
        ...
        return (
            <div >
                ...
                <div>
                    {this.state.isRedirectSignup === true ? <Redirect to='/verifyregister' /> : null}
                </div>
                <Layout>
                    <Header className="head">
                    <h1><Icon type="facebook" />&nbsp;LogIn into Your Account</h1>
                    </Header>
                    <Content>
                    <div className="login-form-container">
                    <h2>LogIn Portal</h2><hr></hr><br></br>
                    {this.state.invalidPassword === true || this.state.unRegisteredEmail === true ? <div className="errMsg">{this.state.errMsg}</div> : <h3>Enter Your Credentials:</h3>}
